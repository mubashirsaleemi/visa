                        ...
                        if (count($services) > 0) {
                            foreach ($services as $q): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?=$q['status']?>
                                        <select name="status" data-id="<?=$q['service_id']?>" class="form-control">
                                    ...
                                    <td class="actions">
                                        ...
                                        <a href="<?=BASEURL?>service/<?=$q['slug']?>" class="btn btn-sm btn-icon btn-pure btn-default on-default edit-row" target="_blank"><i class="icon md-eye" aria-hidden="true"></i></a>
                                    </td>
                                </tr>
                                <?php endforeach;
                        } //end if
...
$(function(){
    $(document).on('change', 'select[name="status"]', function(event) {
        ...
        $status = $this.val();
        $.post('<?=BASEURL."admin/change-service-status"?>', {status: $status, id: $id}, function(resp) {
